...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
        % logZ = 0; % remove this
        % fetch any clique in the uncalibrated tree (there is only one
        % unique clique because we have only two variables X1,X2 in each
        % clique
        product = P.cliqueList(1);
        % now we simply calculate the final belief using the forumla:
        % final_belief = initial_belief * product of messages
        nMsgs = length(unnormalizedMessages);
        for idx = 1:nMsgs
            product = FactorProduct(product,unnormalizedMessages(idx));
        end
        % sum product vals to get Z
        Z = sum(product.val);
        % finally log it
        logZ = log(Z);
    else
