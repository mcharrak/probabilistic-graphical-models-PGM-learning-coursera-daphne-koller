...
% File: RecognizeUnknownActions.m
%
...
for j = 1:nTestActions
    ...
    for m = 1:nModels
        ...
        % generating more data by adding small noise to each pose
        ...
        noisy_poseData = poseData;
        noise_shape = size(poseData(1,:,:));
        noise_shape = noise_shape(2:end);

        for n = 1:N
            curr_poseData = noisy_poseData(n,:,:);
            curr_poseData = squeeze(curr_poseData);
            curr_noise = random('norm', 0, 0.1, noise_shape);
            noisy_poseData(n,:,:) = curr_poseData + curr_noise;
        end
        
        % enlarge dataset
        poseData = [poseData;noisy_poseData];
        N = 2*N;
